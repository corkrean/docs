...
- **Breaking schema change** — the current snapshot is invalidated; you must build a fresh snapshot by re-running the [backfill](./permission-set-lifecycle) at the new revision.
- **Snapshot rotation** — Materialize periodically deploys a new internal snapshot of the origin SpiceDB system as part of routine maintenance. A request against a retired snapshot returns `Aborted: Requested Revision Is No Longer Available`, signalling the consumer to re-run the [backfill](./permission-set-lifecycle) from scratch.
